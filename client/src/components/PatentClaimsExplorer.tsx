...
const categoryMeta = {
  composition: {
    ...
    description: "What the material system is made of and how its constituent layers relate.",
    Icon: Atom,
  },
  manufacturing: {
    text: "text-cyan-300",
    ...
    description: "How the material system is fabricated, assembled, conditioned, or integrated.",
    Icon: Factory,
  },
  device: {
    ...
    description: "How the material system is configured into functional sensing, energy, or device structures.",
    Icon: Cpu,
  },
} satisfies Record<
  ClaimCategory,
  ...
  return (
    ...
    >
      <div className="rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(8,13,21,.9),rgba(4,7,12,.96))] p-4 md:p-5">
        <p className="font-mono text-[9px] tracking-[0.25em] text-primary/70">CLAIM INTELLIGENCE /// EVIDENCE INDEX</p>
      ...
      <p className="sr-only" role="status" aria-live="polite">
        {filteredClaims.length} claim{filteredClaims.length === 1 ? "" : "s"} match the current filters.
      </p>
        ...
        {(Object.keys(categoryMeta) as ClaimCategory[]).map(category => {
          ...
          return (
            <motion.button
              ...
              style={{ boxShadow: active ? `0 24px 70px ${meta.glow}` : undefined }}
            >
              <div
              ...
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <div className={`inline-flex rounded-xl border border-current/15 bg-black/20 p-2.5 ${meta.text}`}>
                    <Icon className="h-5 w-5" aria-hidden />
                  </div>
                  <p className={`mt-4 font-mono text-[9px] tracking-[0.22em] ${meta.text}`}>
                    {category.toUpperCase()} / {String(categoryGroups[category].length).padStart(2, "0")}
                  </p>
                  <h3 className="mt-1 font-display text-base font-semibold text-foreground">{meta.label}</h3>
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{meta.description}</p>
                </div>
                <span className="font-mono text-[9px] text-foreground/35">{active ? "ACTIVE" : "FILTER"}</span>
              </div>
            </motion.button>
          );
        })}
      ...
      <div className="max-h-[36rem] space-y-4 overflow-y-auto pr-2">
          ...
          {filteredClaims.map((claim, idx) => {
            ...
            return (
              <motion.article
                ...
                style={{ boxShadow: isExpanded ? `0 28px 90px ${meta.glow}` : undefined }}
              >
                ...
                >
                  <div className="grid gap-5 md:grid-cols-[auto_1fr_auto] md:items-start">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-xl border border-current/15 bg-black/25 ${meta.text}`}>
                      <Icon className="h-5 w-5" aria-hidden />
                    </div>
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className={`font-mono text-[10px] tracking-[0.2em] ${meta.text}`}>CLAIM {String(claim.number).padStart(2, "0")}</span>
                      ...
                      <h3 className="mt-2 font-display text-lg font-semibold leading-tight text-foreground md:text-xl">{claim.title}</h3>
                      <p className="mt-2 line-clamp-2 text-sm leading-relaxed text-foreground/68">{claim.description}</p>
                    </div>
                    <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: reduceMotion ? 0 : 0.2 }} className={meta.text}>
                    ...
                    >
                      <div className="grid gap-6 border-t border-white/10 p-5 md:grid-cols-[.8fr_1.2fr] md:p-6">
                        <div className="space-y-5">
                          <div>
                            <h4 className="font-mono text-[9px] tracking-[0.22em] text-muted-foreground">CLAIM READING</h4>
                            <p className="mt-2 text-sm leading-relaxed text-foreground/82">{claim.description}</p>
                          </div>
                          <div>
                            <h4 className="font-mono text-[9px] tracking-[0.22em] text-muted-foreground">TECHNICAL PARAMETERS</h4>
                        ...
                        <div className="rounded-2xl border border-white/10 bg-[#050814] p-4 shadow-inner shadow-black/50">
                          <div className="mb-3 flex items-center justify-between gap-3">
                            <div>
                              <h4 className="font-mono text-[9px] tracking-[0.22em] text-muted-foreground">SYSTEM DIAGRAM</h4>
                              <p className="mt-1 text-xs leading-relaxed text-foreground/60">{claim.diagramCaption}</p>
                            </div>
                            <span className={`font-mono text-[9px] ${meta.text}`}>{claim.category.toUpperCase()}</span>
                          </div>
                          <div className="overflow-x-auto rounded-xl border border-white/5 bg-black/25 p-2" dangerouslySetInnerHTML={{ __html: svg }} />
                        </div>
                      </div>
                    </motion.div>
                  ) : null}
        ...
        {filteredClaims.length === 0 ? (
          <p className="py-8 text-center font-mono text-sm text-muted-foreground">No claims match your filters.</p>
        ) : null}
      </div>
    </motion.div>
  );
